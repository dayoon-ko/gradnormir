...
def main(
    ...
    for ood_dataset in "arguana climate-fever cqadupstack dbpedia-entity fiqa nfcorpus quora scidocs scifact trec-covid-v2 webis-touche2020".split():
        print("Dataset:", ood_dataset)
        new_embeddings = torch.load(f"/gallery_louvre/dayoon.ko/research/eval_retrieval/layerwise/embeddings/{model}/{ood_dataset}.pt",
                                weights_only=True)[-1] # Take the last hidden state
        ...
        torch.save(avg_distance_record, f"results/{model}/{ood_dataset}.pt")
