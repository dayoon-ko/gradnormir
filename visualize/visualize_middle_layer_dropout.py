...
class EmbeddingModel:
    ...
    def get_each_encoding_layer_output_with_middle_layer_noise(self, text, noise_level=0.1, add_noise=False):
        ...
        for i in range(len(self.model.encoder.layer)):
            if add_noise and len(self.model.encoder.layer) // 4 == i:
...
if __name__ == "__main__":
    ...
    for text in all_texts:
        embs.append(embedder.get_each_encoding_layer_output_with_middle_layer_noise(text, add_noise=False))
        for i in range(num_aug):
            embs.append(embedder.get_each_encoding_layer_output_with_middle_layer_noise(text, add_noise=True, noise_level=noise_level))
    embs = torch.cat(embs)
