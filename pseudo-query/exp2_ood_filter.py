...
def select_topk(
    ...
    with open(f"results/{model}/{dataset}.jsonl") as f:
        ...
        print(len(target))
    target = [(i, 100) if r == -1 else (i, r) for i, r in enumerate(target)]
    indices = sorted(target, key=lambda x: x[1], reverse=True)[:ood_topk]
    indices = [i for i, _ in indices]
    print(len(indices), "are ood out of", len(target))
        
    
    # Select failed document ids
    with open(f"/gallery_louvre/dayoon.ko/research/sds/src/datasets/{dataset}/queries_generated.jsonl") as f:
        js = [json.loads(i) for i in f.readlines()]
        print(len(js))
    fail_doc_keys = set([js[i]["_id"] for i in indices])
    print("Fail Detected: ", round(len(indices) / len(target) * 100, 
                                   2), "(", len(indices), "out of", len(target), ")")
